...
class TestApi(unittest.TestCase):
    ...
    def test_ner_endpoint_given_json_body_with_known_entities_returns_entity_resul_in_response(self):
        with app.test_client() as client:
            response = client.post('/ner', json={"sentence": "Kamala Harris"})
            data = json.loads(response.get_data())
            print(data)
            assert 'entities' in data, "Response does not contain 'entities' key."
            assert len(data['entities']) > 0, "Entities list is empty."
